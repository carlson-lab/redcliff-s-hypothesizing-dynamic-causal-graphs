...
def kick_off_model_training_experiment(args_dict):
    ...
    curr_run_save_dir = args_dict["save_path"]+os.sep+save_folder_name
    if not os.path.exists(curr_run_save_dir):
        os.mkdir(curr_run_save_dir)
    args_dict["save_path"] = curr_run_save_dir
    print("kick_off_model_training_experiment: curr_run_save_dir == ", curr_run_save_dir)
...
def set_up_and_run_experiments(args_dict, files_of_cached_model_args, files_of_cached_data_args, POSSIBLE_MODEL_TYPES, POSSIBLE_DATA_SETS, shuffle_seed=0):
    ...
    print("set_up_and_run_experiments: [x for x in files_of_cached_model_args if args_dict['model_type'] in x] == ", [x for x in files_of_cached_model_args if args_dict["model_type"]])
    assert len([x for x in files_of_cached_model_args if args_dict["model_type"]]) == 1
    args_dict["model_cached_args_file"] = [x for x in files_of_cached_model_args if args_dict["model_type"]][0]
    ...
    print("set_up_and_run_experiments: UPDATED args_dict == ", args_dict)
...
if __name__=="__main__":
    ...
    parse.add_argument(
        "-cached_args_file",
        default="NAVAR_CLSTM_d4IC_BCTVgs1Parsim_cached_args.txt", # WARNING: edit this line when running new experiments on SLURM server
    ...
    files_of_cached_model_args = [
        "NAVAR_CLSTM_d4IC_BCTVgs1Parsim_cached_args.txt",
    ]
