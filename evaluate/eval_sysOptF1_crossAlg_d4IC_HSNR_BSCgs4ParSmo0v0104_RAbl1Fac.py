...
if __name__=="__main__":
    ...
    parse.add_argument(
        "-cached_args_file",
        default="eval_sysOptF1_crossAlg_d4IC_HSNR_BSCgs4ParSmo0v0104_RAbl1Fac_cached_args.txt", # WARNING: edit this line when running new experiments on SLURM server
    ...
    for cv_id, cv_dset_name in enumerate(CV_DATA_SETS):
        ...
        for f_num in range(NUM_FOLDS_PER_CV_DATASET):
            ...
            for alg_id, alg_name in enumerate(ALGORITHMS_TO_EVALUATE):
                ...
                if "REDCLIFF" in alg_name_alias and trained_model.num_factors_nK == 1:
                    assert len(estimated_gcs) == 1
                    estimated_gcs = [copy.deepcopy(estimated_gcs[0]) for _ in range(len(true_gcs))]
                    assert len(estimated_gcs) == len(true_gcs)
