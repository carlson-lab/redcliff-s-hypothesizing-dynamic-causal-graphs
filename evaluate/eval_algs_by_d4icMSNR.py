...
def prepare_data_for_modeling(orig_data):
    ...
    for i, samp in enumerate(orig_data):
        ...
        if data is None:
            ...
            num_regimes = y.shape[0]
            labels = np.concatenate([y.T for _ in range(T_window_size)], axis=0)
            ...
            labels = np.concatenate([labels]+[y.T for _ in range(T_window_size)], axis=0)
            for r in masks_by_regime_index.keys():
                masks_by_regime_index[r] = np.concatenate([masks_by_regime_index[r], np.zeros(x.shape)], axis=0)
        for r in masks_by_regime_index.keys():
...
if __name__=="__main__":
    ...
    parse.add_argument(
        "-cached_args_file",
        default="eval_algs_by_d4icMSNR_cached_args.txt", # WARNING: edit this line when running new experiments on SLURM server
    ...
    for i,exp_name_key in enumerate(experiments_to_evaluate):
        ...
        for alg_name_key in curr_exp_alg_name_keys:
            ...
            if curr_alg_name in ["PCMCI", "slarac", "qrbs", "lasar"]:
                ...
                if "DCSFA" in curr_alg_name:
                    trained_model = load_model_for_eval(curr_alg_name, trained_alg_path, dynamic_eval=False, d4IC=True)
                else:
                    trained_model = load_model_for_eval(curr_alg_name, trained_alg_path, dynamic_eval=True, d4IC=True)
