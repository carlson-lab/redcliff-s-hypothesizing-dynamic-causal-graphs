"""
Implements the SLARAC (Subsampled Linear Auto-Regression Absolute Coefficients) algorithm.
...
def slarac(data, maxlags=1, n_subsamples=200, subsample_sizes=[INV_GOLDEN_RATIO**(1 / k) for k in [1, 2, 3, 6]],
    ...
    T, N = data.shape
    # Obtain absolute regression coefficients on the entire data set and random subsamples
...
def varmodel(data, maxlags=1, n_samples=None, missing_values=None):
    ...
    Y = data.T[:, maxlags:]
    d = Y.shape[0]
    Z = np.vstack([np.ones((1, Y.shape[1]))] + [data.T[:, maxlags - k:-k] for k in range(1, maxlags + 1)])
    # Subsample data
    ...
    if missing_values is not None:
        keepinds = (np.sum(Y == missing_values, axis=0) + np.sum(Z == missing_values, axis=0)) == 0
        Y = Y[:, keepinds]
        Z = Z[:, keepinds]
    ...
    B[:, :indcutoff] = np.linalg.lstsq(Z[:indcutoff, :].dot(Z[:indcutoff, :].T), Z[:indcutoff, :].dot(Y.T), rcond=None)[0].T
    return B
