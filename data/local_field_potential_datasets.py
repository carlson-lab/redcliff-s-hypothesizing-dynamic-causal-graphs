...
class NormalizedLocalFieldPotentialDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, signal_format, shuffle=True, shuffle_seed=0, max_num_features_per_series=None, dirspec_params=None, grid_search=True, average_region_map=None):
        """
        Notes: 
         - average_region_map takes the form {region1_name: [orig_channel_idx1, orig_channel_idx2, ...], ...}
        """
        super().__init__()
        self.TEMPORAL_DIM = 0
        self.CHANNEL_DIM = 1
        ...
        self.data = []
        files_to_load = [x for x in os.listdir(data_path) if "_subset" in x and ".pkl" in x and "metadata" not in x]

        print("NormalizedLocalFieldPotentialDataset.__init__: loading files from path == ", data_path)
        print("NormalizedLocalFieldPotentialDataset.__init__: files_to_load == ", files_to_load)
        ...
        for i, file_name in enumerate(files_to_load):
            ...
            with open(curr_data_path, 'rb') as infile:
                ...
                for j in range(len(new_samples)):
                    ...
                    if self.average_region_map is not None:
                        curr_data_samp = self.avg_signal_regions_according_to_region_map(curr_data_samp)
                    ...
                    if not np.isnan(np.sum(curr_data_samp)): # case where curr_sample is accepted into dataset (to be normalized later)
                        ...
                        print("NormalizedLocalFieldPotentialDataset.__init__: WARNING - SKIPPING SAMPLE LOCATED AT ", (curr_data_path, j), " ON ACCOUNT OF np.isnan FLAG!!!")
                        num_skipped_samps += 1
            pass

        print("NormalizedLocalFieldPotentialDataset.__init__: TOTAL NUM OF SKIPPED SAMPLES (due to np.isnan flags) IN DATA SET == ", num_skipped_samps)
        print("NormalizedLocalFieldPotentialDataset.__init__: TOTAL NUM SAMPLES IN LOADED DATASET == ", len(self.data))
        ...
        print("NormalizedLocalFieldPotentialDataset.__init__: self.channel_means.shape == ", self.channel_means.shape)
        print("NormalizedLocalFieldPotentialDataset.__init__: self.channel_means == ", self.channel_means)
        print("NormalizedLocalFieldPotentialDataset.__init__: self.channel_std_devs == ", self.channel_std_devs)

        if shuffle:
            random.Random(shuffle_seed).shuffle(self.data)
        
        if grid_search:
            print("NormalizedLocalFieldPotentialDataset.__init__: WARNING!!! ONLY GRABBING 1/10 OF DATASET FOR GRID SEARCH PURPOSES!!!") # FOR GRID SEARCH / DEBUGGING PURPOSES
            self.data = self.data[:len(self.data)//10] # FOR GRID SEARCH / DEBUGGING PURPOSES
            print("NormalizedLocalFieldPotentialDataset.__init__: WARNING!!! SHORTENED len(self.data) == ", len(self.data), " !!!") # FOR GRID SEARCH / DEBUGGING PURPOSES
        pass
        
    def __len__(self):
        return len(self.data)
    
    def avg_signal_regions_according_to_region_map(self, signal):
        """
        Notes: 
         - signal takes the form of a numpy array with shape (num_time_steps, num_channels) originally
         - average_region_map takes the form {region1_name: [orig_channel_idx1, orig_channel_idx2, ...], ...}
         
        returns:
         - region_avgd_signal, which is a numpy array of shape (num_time_steps, num_regions)
        """
        assert self.average_region_map is not None
        num_regions_to_avg = len(self.average_region_map.keys())
        region_avgd_signal = np.zeros((signal.shape[0], num_regions_to_avg))
        for i,region_name in enumerate(self.average_region_map.keys()):
            region_avgd_signal[:,i] = np.mean(signal[:,self.average_region_map[region_name]], axis=1)
        return region_avgd_signal
    ...
    def __getitem__(self, index, label_each_time_step=False, torch_sample=False):
        ...
        with open(self.data[index][0], 'rb') as infile:
            ...
            if self.average_region_map is not None:
                assert not torch_sample
                x = self.avg_signal_regions_according_to_region_map(x)
            if torch_sample:
            ...
            if "directed_spectrum" in self.signal_format:
                ...
                if self.freq_bins is None:
                    ...
                    print("NormalizedLocalFieldPotentialDataset.__getitem__: len(self.freq_bins) == ", len(self.freq_bins))
                    print("NormalizedLocalFieldPotentialDataset.__getitem__: self.freq_bins == ", self.freq_bins)
            ...
            if len(y.size()) != 2 and label_each_time_step: # copy y label for every time point in x
                ...
                raise NotImplementedError("NormalizedLocalFieldPotentialDataset.__init__: CAN'T CURRENTLY HANDLE y.size() == "+str(y.size())+" when x.size() == "+str(x.size()))
        
        return x.float(), y.float()


def load_normalized_lfp_data(data_path, batch_size, signal_format="original", shuffle=True, shuffle_seed=0, dirspec_params=None, grid_search=True, average_region_map=None):
    data_loader = None
    if data_path is not None:
        data_set = NormalizedLocalFieldPotentialDataset(data_path, signal_format=signal_format, shuffle=shuffle, shuffle_seed=shuffle_seed, dirspec_params=dirspec_params, grid_search=grid_search, average_region_map=average_region_map)
        data_loader = DataLoader(data_set, batch_size=batch_size) # see https://blog.paperspace.com/dataloaders-abstractions-pytorch/
    return data_loader


def load_normalized_lfp_data_train_test_split(data_root_path, batch_size, signal_format="original", shuffle=True, shuffle_seed=0, train_portion=0.8, dirspec_params=None, grid_search=True, average_region_map=None):
    ...
    train_set = NormalizedLocalFieldPotentialDataset(train_data_path, signal_format=signal_format, shuffle=shuffle, shuffle_seed=shuffle_seed, dirspec_params=dirspec_params, grid_search=grid_search, average_region_map=average_region_map)
    train_loader = DataLoader(train_set, batch_size=batch_size) # see https://blog.paperspace.com/dataloaders-abstractions-pytorch/
    val_set = NormalizedLocalFieldPotentialDataset(val_data_path, signal_format=signal_format, shuffle=shuffle, shuffle_seed=shuffle_seed, dirspec_params=dirspec_params, grid_search=grid_search, average_region_map=average_region_map)
    val_loader = DataLoader(val_set, batch_size=batch_size) # see https://blog.paperspace.com/dataloaders-abstractions-pytorch/
    return train_loader, val_loader
    

def load_normalized_lfp_data_train_test_split_as_matrices(data_root_path, signal_format="original flattened", shuffle=True, shuffle_seed=0, 
                                                            max_num_features_per_series=25, train_portion=0.8, dirspec_params=None, grid_search=True, average_region_map=None):
    train_data_path = data_root_path+os.sep+"train"
    val_data_path = data_root_path+os.sep+"validation"
    ...
    train_set = NormalizedLocalFieldPotentialDataset(
        train_data_path, 
    ...
    print("data.local_field_potential_datasets.load_normalized_lfp_data_train_test_split_as_matrices: len(train_set) == ", len(train_set))
    X_train = None
    Y_train = None
    for i in range(len(train_set)):
        ...
        y = y.squeeze()
        if X_train is None:
            print("data.local_field_potential_datasets.load_normalized_lfp_data_train_test_split_as_matrices: train x.size() == ", x.size())
            print("data.local_field_potential_datasets.load_normalized_lfp_data_train_test_split_as_matrices: train y.size() == ", y.size())
    ...
    val_set = NormalizedLocalFieldPotentialDataset(
        val_data_path, 
        signal_format=signal_format, 
        shuffle=shuffle, 
        shuffle_seed=shuffle_seed, 
    ...
    print("data.local_field_potential_datasets.load_normalized_lfp_data_train_test_split_as_matrices: len(val_set) == ", len(val_set))
    X_val = None
    Y_val = None
    for i in range(len(val_set)):
        ...
        y = y.squeeze()
        if X_val is None:
            print("data.local_field_potential_datasets.load_normalized_lfp_data_train_test_split_as_matrices: val x.size() == ", x.size())
            print("data.local_field_potential_datasets.load_normalized_lfp_data_train_test_split_as_matrices: val y.size() == ", y.size())
