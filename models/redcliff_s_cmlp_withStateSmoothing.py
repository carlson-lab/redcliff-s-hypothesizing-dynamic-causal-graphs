...
class REDCLIFF_S_CMLP_withStateSmoothing(nn.Module):
    def __init__(self, num_chans, gen_lag, gen_hidden, embed_lag, embed_hidden_sizes, num_in_timesteps, num_out_timesteps,
                 ...
                 num_acclimation_epochs=0, STATE_SCORE_SMOOTHING_EPSILON=0.0001):
        '''
        REDCLIFF_S_CMLP_withStateSmoothing model with num_factors cMLP-based factors per time series.
        '''
        super(REDCLIFF_S_CMLP_withStateSmoothing, self).__init__()
        self.MAX_NUM_SAMPS_FOR_GC_VIS = 5
        self.MAX_NUM_SAMPS_FOR_GC_PROGRESS_TRACKING = 40
        self.STATE_SCORE_SMOOTHING_EPSILON = STATE_SCORE_SMOOTHING_EPSILON
        ...
        self.FACTOR_WEIGHT_SMOOTHING_PENALTY_COEFF = coeff_dict["FACTOR_WEIGHT_SMOOTHING_PENALTY_COEFF"]
        ...
        if factor_score_embedder_type == "cEmbedder":
            ...
            print("REDCLIFF_S_CMLP_withStateSmoothing.__init__: factor_score_embedder_args == ", factor_score_embedder_args)
    ...
    def initialize_factors_with_prior(self, prior_factors_path=None, X_train=None, cost_criteria="CosineSimilarity", unsupervised_start_index=0, max_batches=10):
        ...
        if X_train is not None:
            ...
            _, matched_est_inds, matched_ground_truth_inds = sort_unsupervised_estimates(
                labels_across_recordings_by_factor, 
                labels_across_recordings_by_ground_truth, 
    ...
    def resume_training_from_checkpoint(self, training_meta_data_path):
        print("REDCLIFF_S_CMLP_withStateSmoothing.resume_training_from_checkpoint: RESUMING TRAINING FROM CHKPT SAVED AT training_meta_data_path==", training_meta_data_path)
        with open(training_meta_data_path, 'rb') as infile:
            ...
            self.chkpt_avg_fw_smoothing_penalty = training_checkpoint_meta_data["avg_fw_smoothing_penalty"]
            ...
            if self.num_supervised_factors > 0:
                self.chkpt_factor_score_train_acc_history = training_checkpoint_meta_data["factor_score_train_acc_history"] # FOR CLASSIFIER DEBUGGING
                self.chkpt_factor_score_train_tpr_history = training_checkpoint_meta_data["factor_score_train_tpr_history"] # FOR CLASSIFIER DEBUGGING
                self.chkpt_factor_score_train_tnr_history = training_checkpoint_meta_data["factor_score_train_tnr_history"] # FOR CLASSIFIER DEBUGGING
                self.chkpt_factor_score_train_fpr_history = training_checkpoint_meta_data["factor_score_train_fpr_history"] # FOR CLASSIFIER DEBUGGING
                self.chkpt_factor_score_train_fnr_history = training_checkpoint_meta_data["factor_score_train_fnr_history"] # FOR CLASSIFIER DEBUGGING
                self.chkpt_factor_score_val_acc_history = training_checkpoint_meta_data["factor_score_val_acc_history"] # FOR CLASSIFIER DEBUGGING
                self.chkpt_factor_score_val_tpr_history = training_checkpoint_meta_data["factor_score_val_tpr_history"] # FOR CLASSIFIER DEBUGGING
                self.chkpt_factor_score_val_tnr_history = training_checkpoint_meta_data["factor_score_val_tnr_history"] # FOR CLASSIFIER DEBUGGING
                self.chkpt_factor_score_val_fpr_history = training_checkpoint_meta_data["factor_score_val_fpr_history"] # FOR CLASSIFIER DEBUGGING
                self.chkpt_factor_score_val_fnr_history = training_checkpoint_meta_data["factor_score_val_fnr_history"] # FOR CLASSIFIER DEBUGGING
        ...
        print("REDCLIFF_S_CMLP_withStateSmoothing.resume_training_from_checkpoint: RESUMING TRAINING FROM CHKPT MADE AT chkpt_epoch==", self.chkpt_epoch)
        print("REDCLIFF_S_CMLP_withStateSmoothing.resume_training_from_checkpoint: RESUMING TRAINING WITH MODEL PARAMETERS SAVED AT chkpt_best_it==", self.chkpt_best_it)
        print("REDCLIFF_S_CMLP_withStateSmoothing.resume_training_from_checkpoint: WARNING - OPTIMIZERS HAVE NOT BEEN CHECKPOINTED, MEANINIG THAT RESUMED TRAINING WILL BEGIN WITH FRESH OPTIMIZERS/GRADIENT DESCENT PARAMETERS THAT WOULD NEED TO BE MENTIONED IN ANY REPORTS", flush=True)
        pass
    ...
    def forward(self, X, factor_weightings=None):
        ...
        if self.forward_pass_mode == "apply_factor_weights_at_each_sim_step":
            ...
            raise ValueError("REDCLIFF_S_CMLP_withStateSmoothing.forward: UNRECOGNIZED self.forward_pass_mode == "+str(self.forward_pass_mode))
    ...
    def compute_loss(self, conditioning_X, preds, targets, factor_scores, factor_labels, gc_est_mode, node_dag_scale=0.1, embedder_pretrain_loss=False, factor_pretrain_loss=False):
            ...
            if len(factor_labels.size()) == 3:
                if factor_labels.size()[2] > max(self.gen_lag, self.embed_lag):
                    for y, yhat in zip([factor_labels[:,:,max(self.gen_lag, self.embed_lag)+l] for l in range(factor_labels.size()[2]-max(self.gen_lag, self.embed_lag))],factor_scores):
                        factor_loss = factor_loss + self.FACTOR_SCORE_COEFF*self.supervised_loss_fn(
                            yhat[:,:self.num_supervised_factors], 
                            y[:,:self.num_supervised_factors]
                        )
        ...
        fw_smoothing_penalty = torch.tensor([0.0], requires_grad=True) # see https://discuss.pytorch.org/t/how-to-initialize-zero-loss-tensor/86888/2
        if torch.cuda.is_available():
            fw_smoothing_penalty = fw_smoothing_penalty.to(device="cuda")
        if self.num_sims == 2:
            state_score_diff = factor_scores[0] - factor_scores[1]
            state_score_diff_mask = state_score_diff > self.STATE_SCORE_SMOOTHING_EPSILON
            state_score_diff_mask.requires_grad_(False) # see https://discuss.pytorch.org/t/how-to-turn-off-requires-grad-for-individual-params/103303/2
            state_score_diff = state_score_diff * state_score_diff_mask
            fw_smoothing_penalty = torch.sum(state_score_diff**2.)
        elif self.num_sims > 2:
            for interval_ind, (state_score_t0, state_score_t1, state_score_t2) in enumerate(zip(factor_scores[:-2], factor_scores[1:-1], factor_scores[2:])):
                score_diff_over_full_interval = state_score_t2 - state_score_t0
                score_2v1_diff = state_score_t2 - state_score_t1
                score_2v1_diff_mask = torch.gt(torch.abs(score_2v1_diff), torch.abs(score_diff_over_full_interval)) # see https://pytorch.org/docs/stable/generated/torch.gt.html
                score_2v1_diff_mask.requires_grad_(False) # see https://discuss.pytorch.org/t/how-to-turn-off-requires-grad-for-individual-params/103303/2
                score_2v1_diff = score_2v1_diff * score_2v1_diff_mask
                fw_smoothing_penalty = fw_smoothing_penalty + torch.sum(score_2v1_diff**2.)
                if interval_ind == 0:
                    score_1v0_diff = state_score_t1 - state_score_t0
                    score_1v0_diff_mask = torch.gt(torch.abs(score_1v0_diff), torch.abs(score_diff_over_full_interval)) # see https://pytorch.org/docs/stable/generated/torch.gt.html
                    score_1v0_diff_mask.requires_grad_(False) # see https://discuss.pytorch.org/t/how-to-turn-off-requires-grad-for-individual-params/103303/2
                    score_1v0_diff = score_1v0_diff * score_1v0_diff_mask
                    fw_smoothing_penalty = fw_smoothing_penalty + torch.sum(score_1v0_diff**2.)
        fw_smoothing_penalty = fw_smoothing_penalty * self.FACTOR_WEIGHT_SMOOTHING_PENALTY_COEFF
        ...
        for sample_ind in range(len(gc)):
            if sample_ind == 0:
                ...
                for f_ind, f_Adj in enumerate(gc_lagged[sample_ind]):
                    if f_ind == 0:
                        adj_l1_penalty = self.ADJ_L1_REG_COEFF*sum(
                            [torch.log(torch.tensor(i+2.))*torch.norm(f_Adj[:,:,i], 1) for i in range(f_Adj.size()[2])]
        ...
        if embedder_pretrain_loss:
            ...
            combo_loss = factor_loss + fw_l1_penalty + fw_smoothing_penalty
        elif factor_pretrain_loss:
            combo_loss = forecasting_loss + fw_l1_penalty + fw_smoothing_penalty + adj_l1_penalty
            if factor_cos_sim_penalty is not None:
                combo_loss += factor_cos_sim_penalty
        else:
            combo_loss = forecasting_loss + factor_loss + fw_l1_penalty + fw_smoothing_penalty + adj_l1_penalty
            if factor_cos_sim_penalty is not None:
                combo_loss += factor_cos_sim_penalty
        
        return combo_loss, [forecasting_loss, factor_loss, factor_cos_sim_penalty, fw_l1_penalty, fw_smoothing_penalty, adj_l1_penalty, reg_dagness_loss]

    
    def batch_update(self, epoch_num, batch_num, X, Y, optimizerA, optimizerB, output_length, best_model=None, 
                     training_status_of_each_factor=None, running_factor_score_confusion_matrix=None): 
        # Set up data.
        if torch.cuda.is_available():
            X = X.to(device="cuda")
            Y = Y.to(device="cuda")
        ...
        if currently_pretraining_embedder:
            ...
            if running_factor_score_confusion_matrix is not None: 
                ...
                running_factor_score_confusion_matrix += confusion_matrix(conf_mat_labels, conf_mat_preds, labels=[i for i in range(self.num_supervised_factors)]) # FOR CLASSIFIER DEBUGGING
        ...
        if currently_training_in_combination:
            ...
            if running_factor_score_confusion_matrix is not None: # FOR CLASSIFIER DEBUGGING
                state_label_pred_mask = state_label_preds[0].detach().cpu().numpy()
                Y_mask = None
                if len(Y.size()) == 3:
                    if  Y.size()[2] > max(self.gen_lag, self.embed_lag):
    ...
    def save_checkpoint(self, save_dir, it, best_model, avg_forecasting_loss, avg_factor_loss, avg_factor_cos_sim_penalty, avg_fw_l1_penalty, 
                        avg_fw_smoothing_penalty, avg_adj_penalty, avg_dagness_reg_loss, avg_dagness_lag_loss, avg_dagness_node_loss, 
                        avg_combo_loss, best_loss, best_it, f1score_histories, f1score_OffDiag_histories, roc_auc_histories, roc_auc_OffDiag_histories, 
                        gc_factor_l1_loss_histories, gc_factor_cosine_sim_histories, gc_factorUnsupervised_cosine_sim_histories, deltacon0_histories, 
                        deltacon0_with_directed_degrees_histories, deltaffinity_histories, path_length_mse_histories, GC, X_vis, output_length=None, 
                        num_sim_steps=None, factor_score_train_acc_history=None, factor_score_train_tpr_history=None, factor_score_train_tnr_history=None, 
                        factor_score_train_fpr_history=None, factor_score_train_fnr_history=None, factor_score_val_acc_history=None, 
                        factor_score_val_tpr_history=None, factor_score_val_tnr_history=None, factor_score_val_fpr_history=None, factor_score_val_fnr_history=None,):
        # save summary stats
        ...
        with open(meta_data_save_path, "wb") as outfile:
            pkl.dump({
                ...
                "avg_fw_smoothing_penalty": avg_fw_smoothing_penalty, 
                ...
                "factor_score_train_acc_history": factor_score_train_acc_history, # FOR CLASSIFIER DEBUGGING 
                "factor_score_train_tpr_history": factor_score_train_tpr_history, # FOR CLASSIFIER DEBUGGING 
                "factor_score_train_tnr_history": factor_score_train_tnr_history, # FOR CLASSIFIER DEBUGGING 
                "factor_score_train_fpr_history": factor_score_train_fpr_history, # FOR CLASSIFIER DEBUGGING 
                "factor_score_train_fnr_history": factor_score_train_fnr_history, # FOR CLASSIFIER DEBUGGING 
                "factor_score_val_acc_history": factor_score_val_acc_history, # FOR CLASSIFIER DEBUGGING 
                "factor_score_val_tpr_history": factor_score_val_tpr_history, # FOR CLASSIFIER DEBUGGING 
                "factor_score_val_tnr_history": factor_score_val_tnr_history, # FOR CLASSIFIER DEBUGGING 
                "factor_score_val_fpr_history": factor_score_val_fpr_history, # FOR CLASSIFIER DEBUGGING 
                "factor_score_val_fnr_history": factor_score_val_fnr_history, # FOR CLASSIFIER DEBUGGING
        ...
        plot_curve(avg_factor_loss, "Validation Factor Score MSE Loss", "Epoch", "Average MSE Loss", save_dir+os.sep+"avg_val_factor_score_mse_loss.png", domain_start=0)
        plot_curve(avg_factor_cos_sim_penalty, "Validation Factor Cosine Similarity Penalty", "Epoch", "Average Penalty", save_dir+os.sep+"avg_factor_cos_sim_penalty.png", domain_start=0)
        plot_curve(avg_fw_l1_penalty, "Validation Factor Weight L1 Penalty", "Epoch", "Average L1 Penalty", save_dir+os.sep+"avg_val_fw_L1_penalty.png", domain_start=0)
        plot_curve(avg_fw_smoothing_penalty, "Validation Factor Weight Smoothing Penalty", "Epoch", "Average Smoothing Penalty", save_dir+os.sep+"avg_val_fw_smoothing_penalty.png", domain_start=0)
        ...
        plot_curve_comparisson(
            gc_factor_l1_loss_histories, "GC L1 Loss History", "Epoch", "L1 Norm", save_dir+os.sep+"gc_l1_loss_history_visualization.png", 
        ...
        plot_curve_comparisson_from_dict(
            gc_factorUnsupervised_cosine_sim_histories, "Unsupervised GC Cosine Similarity History", "Epoch", "Cosine Similarity", 
            save_dir+os.sep+"gc_factorUnsupervised_cosine_sim_histories_visualization.png", domain_start=0, label_root="factors"
        )
        
        plot_curve_comparisson(
            deltacon0_histories, "GC DeltaCon0 Similarity", "Epoch", "DeltaCon0 Similarity", save_dir+os.sep+"gc_deltacon0_similarity_history_vis.png", 
            domain_start=0, label_root="factor"
        )
        plot_curve_comparisson(
            deltacon0_with_directed_degrees_histories, "GC DeltaCon0-with-Directed-Degrees Similarity", "Epoch", "DeltaCon0-wDD Similarity", 
            save_dir+os.sep+"gc_deltacon0_wDD_similarity_history_vis.png", domain_start=0, label_root="factor"
        )
        plot_curve_comparisson(
        ...
        for batch_num, (X, _) in enumerate(X_vis):
            ...
            for samp_ind in range(min(len(GC_est), self.MAX_NUM_SAMPS_FOR_GC_VIS)):
                ...
                plot_gc_est_comparissons_by_factor(GC_noLags, curr_GC_est, save_dir+os.sep+"gc_est_noLags_results_epoch"+str(it)+"_sampInd"+str(samp_ind)+".png")
    ...
    def determine_which_factors_need_updates(self, cached_model, training_status_of_each_factor):
        cached_model_gc_ests_NoLag = [
            x.detach().cpu().numpy() for x in cached_model.GC(
                "fixed_factor_exclusive", X=None, threshold=False, ignore_lag=True, combine_wavelet_representations=False, rank_wavelets=False
            )[0]
        ]
    ...
    def fit(self, save_dir, X_train, optimizerA, optimizerB, input_length, output_length, num_sim_steps, max_iter, X_val, lookback=5, check_every=50, 
            verbose=1, GC=None, deltaConEps=0.1, in_degree_coeff=1., out_degree_coeff=1., prior_factors_path=None, cost_criteria="CosineSimilarity", 
            unsupervised_start_index=0, max_factor_prior_batches=10, stopping_criteria_forecast_coeff=1., stopping_criteria_factor_coeff=1., 
            stopping_criteria_cosSim_coeff=1.):
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: START", flush=True)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: WARNING - NOT USING THE input_length ARGUMENT (FROM A DECRIMENTED IMPLEMENTATION), AND INSTEAD USING gen_lag AND embed_lag ARGUMENTS PASSED TO __init__ !!!", flush=True)
        ...
        gc_factor_cosine_sim_histories = {str(i)+"and"+str(j):[] for i in range(self.num_supervised_factors) for j in range(self.num_supervised_factors) if i < j}
        gc_factorUnsupervised_cosine_sim_histories = {str(i)+"and"+str(j):[] for i in range(self.num_supervised_factors, self.num_factors_nK) for j in range(self.num_supervised_factors, self.num_factors_nK) if i < j}
        ...
        path_length_mse_histories = {path_length:[[] for _ in range(self.num_supervised_factors)] for path_length in range(1,self.num_chans)}
        ...
        try:
            ...
            avg_fw_smoothing_penalty = self.chkpt_avg_fw_smoothing_penalty[:iter_start]
            ...
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: RESUMING TRAINING WITH THE FOLLOWING CHKPT CONFIGURATION: ")
        except:
            if RESUMING_TRAININIG:
                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: WARNING - EXCEPTION ENCOUNTERED WHEN TRYNING TO RESUME TRAINING, WITH CHKPT PARTIALLY LOADED! THE CURRENT CHKPT CONFIGURATION IS: ")
            pass
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t iter_start == ", iter_start)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_forecasting_loss == ", avg_forecasting_loss)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_factor_loss == ", avg_factor_loss)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_factor_cos_sim_penalty == ", avg_factor_cos_sim_penalty)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_fw_l1_penalty == ", avg_fw_l1_penalty)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_fw_smoothing_penalty == ", avg_fw_smoothing_penalty)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_adj_penalty == ", avg_adj_penalty)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_dagness_reg_loss == ", avg_dagness_reg_loss)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_dagness_lag_loss == ", avg_dagness_lag_loss)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_dagness_node_loss == ", avg_dagness_node_loss)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t avg_combo_loss == ", avg_combo_loss)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t best_loss == ", best_loss)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t best_it == ", best_it)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t f1score_histories == ", f1score_histories)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t f1score_OffDiag_histories == ", f1score_OffDiag_histories)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t roc_auc_histories == ", roc_auc_histories)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t roc_auc_OffDiag_histories == ", roc_auc_OffDiag_histories)
        if self.num_supervised_factors > 0:
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_train_acc_history == ", factor_score_train_acc_history) 
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_train_tpr_history == ", factor_score_train_tpr_history) 
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_train_tnr_history == ", factor_score_train_tnr_history) 
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_train_fpr_history == ", factor_score_train_fpr_history) 
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_train_fnr_history == ", factor_score_train_fnr_history) 
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_val_acc_history == ", factor_score_val_acc_history) 
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_val_tpr_history == ", factor_score_val_tpr_history) 
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_val_tnr_history == ", factor_score_val_tnr_history) 
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_val_fpr_history == ", factor_score_val_fpr_history) 
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t factor_score_val_fnr_history == ", factor_score_val_fnr_history) 
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t gc_factor_l1_loss_histories == ", gc_factor_l1_loss_histories)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t gc_factor_cosine_sim_histories == ", gc_factor_cosine_sim_histories)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t gc_factorUnsupervised_cosine_sim_histories == ", gc_factorUnsupervised_cosine_sim_histories)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t deltacon0_histories == ", deltacon0_histories)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t deltacon0_with_directed_degrees_histories == ", deltacon0_with_directed_degrees_histories)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t deltaffinity_histories == ", deltaffinity_histories)
        print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t\t path_length_mse_histories == ", path_length_mse_histories, flush=True)

        for it in range(iter_start, max_iter):
            print("REDCLIFF_S_CMLP_withStateSmoothing.fit: now on epoch it == ", it, flush=True)
            if (it == self.num_pretrain_epochs and "pretrain_factor" in self.training_mode) or (prior_factors_path is not None and it == 0): # end of pretraining
                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: <<< NOW USING PRIOR TO INITIALIZE/RESET FACTORS >>>", flush=True)
                self.initialize_factors_with_prior(
                    prior_factors_path=prior_factors_path, 
                    X_train=X_train, 
                    cost_criteria=cost_criteria, 
                    unsupervised_start_index=unsupervised_start_index, 
            ...
            if self.num_supervised_factors > 0:
                ...
                avg_val_dagness_lag_loss, \
                avg_val_dagness_node_loss, \
                avg_val_combo_loss = self.validate_training(
                    X_val, output_length, self.num_series
                )
            ...
            avg_fw_smoothing_penalty.append(avg_val_fw_smoothing_penalty)
            ...
            if it >= self.num_pretrain_epochs+self.num_acclimation_epochs:
                ...
                if "Freeze" in self.training_mode:
                    assert best_model is not None
                    need_to_change_out_factor = self.determine_which_factors_need_updates(best_model, training_status_of_each_factor)
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: need_to_change_out_factor == ", need_to_change_out_factor)
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: orig training_status_of_each_factor == ", training_status_of_each_factor)
                    ...
                    for f_ind in range(self.num_factors_nK):
                        if "Epoch" in self.training_mode and training_status_of_each_factor[f_ind]:
                            ...
                            if need_to_change_out_factor[f_ind]:
                                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: recording improved version of factor ", f_ind)
                                best_model.factors[f_ind] = deepcopy(self.factors[f_ind]) # update record of current factor
                                update_cached_gen_model_factors = True
                                update_cached_factor_score_embedder = True
                            else:
                                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: reverting to previous version of factor ", f_ind)
                                self.factors[f_ind] = deepcopy(best_model.factors[f_ind]) # revert to prior saved version of current factor
                    ...
                    if update_cached_factor_score_embedder:
                        best_model.factor_score_embedder = deepcopy(self.factor_score_embedder) # we always keep the most up-to-date factor score embedder so as to avoid adverse effects on any newly-frozen factors
                        best_model.gen_model[0] = best_model.factor_score_embedder
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: new training_status_of_each_factor == ", training_status_of_each_factor)
                    ...
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: NON-FROZEN curr_criteria_val == ", curr_criteria_val, flush=True)
                    
                    # apply "Freeze" training mode stopping criteria
                    if (sum(training_status_of_each_factor) > 0) or curr_criteria_val < best_loss: # FOR DEBUGGING PURPOSES
                    ...
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: avg_val_factor_loss == ", avg_val_factor_loss)
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: avg_val_forecasting_loss == ", avg_val_forecasting_loss)
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: curr_cosSim_mean == ", curr_cosSim_mean)
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: it == ", it)
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: best_it == ", best_it, flush=True)
                    curr_criteria_val = None
                    if self.num_supervised_factors > 0:
                        if self.num_supervised_factors > 1:
                            curr_criteria_val = (stopping_criteria_factor_coeff*avg_val_factor_loss)+(stopping_criteria_forecast_coeff*avg_val_forecasting_loss)+(stopping_criteria_cosSim_coeff*curr_cosSim_mean)
                        ...
                        curr_criteria_val = stopping_criteria_forecast_coeff*avg_val_forecasting_loss
                    print("REDCLIFF_S_CMLP_withStateSmoothing.fit: curr_criteria_val == ", curr_criteria_val, flush=True)
                ...
                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t WRAPPING UP PRE-TRAINING EPOCH ", it, "; STOPPING CRITERION WILL NOT BE TRACKED/EVALUATED UNTIL PRE-TRAINING IS COMPLETE, BUT THE MODEL/EPOCH WILL.", flush=True)
                best_it = it
                best_model = deepcopy(self)
            ...
            if it % check_every == 0:
                ...
                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t CHECKING")
                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t avg_forecasting_loss == ", avg_forecasting_loss)
                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t avg_factor_loss == ", avg_factor_loss)
                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t avg_factor_cos_sim_penalty == ", avg_factor_cos_sim_penalty)
                print("REDCLIFF_S_CMLP_withStateSmoothing.fit: \t avg_fw_l1_penalty == ", avg_fw_l1_penalty)
                ...
                if self.num_supervised_factors > 0:
                    self.save_checkpoint(
                        ...
                        avg_fw_smoothing_penalty, avg_adj_penalty, avg_dagness_reg_loss, avg_dagness_lag_loss, avg_dagness_node_loss, 
                        avg_combo_loss, best_loss, best_it, f1score_histories, f1score_OffDiag_histories, roc_auc_histories, 
                        roc_auc_OffDiag_histories, gc_factor_l1_loss_histories, gc_factor_cosine_sim_histories, 
                        gc_factorUnsupervised_cosine_sim_histories, deltacon0_histories, deltacon0_with_directed_degrees_histories, 
                        deltaffinity_histories, path_length_mse_histories, GC, X_val, output_length=None, num_sim_steps=None, 
        ...
        if self.num_supervised_factors > 0:
            ...
            _, _, _, _, _, _, _, _, _, final_mean_val_combo_loss = self.validate_training(X_val, output_length, self.num_series)
    ...
    def validate_training(self, X_val, output_length, num_series, factor_score_val_acc_history=None, factor_score_val_tpr_history=None, 
        ...
        for batch_num, (X, Y) in enumerate(X_val):
            ...
            x_sims_genUpdate, _, _, state_label_preds = self.forward(X[:,:max(self.gen_lag, self.embed_lag),:])
            # Calculate loss 
            combined_loss, [forecasting_loss, factor_loss, factor_cos_sim_penalty, fw_l1_penalty, fw_smoothing_penalty, adj_l1_penalty, reg_dagness_loss] = self.compute_loss(
                X[:,:self.embed_lag,:], 
                x_sims_genUpdate, 
                X[:,max(self.gen_lag, self.embed_lag):max(self.gen_lag, self.embed_lag)+(self.num_sims*output_length),:],
                state_label_preds,
            ...
            fw_smoothing_penalty = fw_smoothing_penalty.cpu().detach().item()
            ...
            avg_forecasting_loss += forecasting_loss
            ...
            if self.FACTOR_WEIGHT_SMOOTHING_PENALTY_COEFF > 0.: 
                fw_smoothing_penalty /= self.FACTOR_WEIGHT_SMOOTHING_PENALTY_COEFF
            avg_fw_smoothing_penalty += fw_smoothing_penalty
        ...
        avg_fw_smoothing_penalty = avg_fw_smoothing_penalty/len(X_val)
        ...
        if self.num_supervised_factors > 0:
            return avg_forecasting_loss, avg_factor_loss, avg_factor_cos_sim_penalty, avg_fw_l1_penalty, avg_fw_smoothing_penalty, avg_adj_penalty, avg_dagness_reg_loss, avg_dagness_lag_loss, avg_dagness_node_loss, avg_combo_loss, factor_score_val_acc_history, factor_score_val_tpr_history, factor_score_val_tnr_history, factor_score_val_fpr_history, factor_score_val_fnr_history 
        return avg_forecasting_loss, avg_factor_loss, avg_factor_cos_sim_penalty, avg_fw_l1_penalty, avg_fw_smoothing_penalty, avg_adj_penalty, avg_dagness_reg_loss, avg_dagness_lag_loss, avg_dagness_node_loss, avg_combo_loss
