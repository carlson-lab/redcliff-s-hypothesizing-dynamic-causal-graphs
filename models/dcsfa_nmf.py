...
from general_utils.misc import unflatten_directed_spectrum_features
...
class NmfBase(nn.Module):
    ...
    * ``ValueError``: if fixed corr values are not in {"positive","negative","n/a"}
    ...
    def __init__(self, n_components, device="auto", n_sup_networks=1, fixed_corr=None, recon_loss="MSE", recon_weight=1.0, 
                 sup_recon_type="Residual", sup_recon_weight=1.0, sup_smoothness_weight=1, feature_groups=None, 
                 group_weights=None, verbose=False,):
    ...
    def residual_loss_f(self, s, s_h):
        """
        Loss function between supervised factor scores and the maximal values for reconstruction. Factors scores are encouraged 
        to be non-zero by the smoothness weight.
...
class DcsfaNmf(NmfBase):
    ...
    def fit(self, X, y, y_pred_weights=None, task_mask=None, intercept_mask=None, y_sample_groups=None, n_epochs=100, n_pre_epochs=100, 
        ...
        for epoch in epoch_iter:
            ...
            with torch.no_grad():
                self.eval()
                X_recon, y_pred, _ = self.transform(X, intercept_mask, avg_intercept=False, return_npy=True,)
                ...
                if X_val is not None and y_val is not None:
                    X_recon_val, y_pred_val, _ = self.transform(X_val, return_npy=True,)
                    ...
                    if mse_var_rat + auc_err < self.best_performance:
                        ...
                        torch.save(self, self.save_folder +os.sep+ self.best_model_name,)
...
class FullDCSFAModel(DcsfaNmf):
    ...
    def get_factor_GC(self, factor, threshold=True, ignore_features=True):
        node_factor_len = self.num_high_level_node_features*(2*self.num_nodes - 1)
        assert factor.shape[1] == self.num_nodes*node_factor_len 
        node_subfactors = np.zeros((self.num_nodes, node_factor_len))
        for i in range(self.num_nodes):
            node_subfactors[i,:] = node_subfactors[i,:] + factor[0,i*node_factor_len:(i+1)*node_factor_len]
        raw_adjacency_tensor = unflatten_directed_spectrum_features(node_subfactors)
    ...
    def evaluate(self, X_orig, y_orig, GC_orig, save_path, threshold=False, ignore_features=True):
        ...
        print("models.dcsfa_nmf.FullDCSFAModel.evaluate: gc_mse == ", gc_mse)
        ...
        print("models.dcsfa_nmf.FullDCSFAModel.evaluate: recon_mse == ", recon_mse)
        print("models.dcsfa_nmf.FullDCSFAModel.evaluate: score_mse == ", score_mse)
        avg_recon_mse = torch.mean(recon_mse)
        avg_score_mse = torch.mean(score_mse)
        print("models.dcsfa_nmf.FullDCSFAModel.evaluate: avg_recon_mse == ", avg_recon_mse)
        print("models.dcsfa_nmf.FullDCSFAModel.evaluate: avg_score_mse == ", avg_score_mse)
