# FROM THE AUTHOR(S): THE FOLLOWING CODE IS ADAPTED FROM https://github.com/mckinsey/causalnex/blob/develop/causalnex/structure/dynotears.py such that it can be used in our experiments; aspects of the code which differ from the original implementation (which itself falls under the Apache 2.0 license) are marked with a '<><><>' comment
...
def _learn_dynamic_structure(
    ...
    for n_iter in range(max_iter):
        ...
        alpha += rho * h_value
        if h_value <= h_tol:
            print("causalnex_dynotears_vanilla._learn_dynamic_structure: FINAL n_iter == ", n_iter, flush=True) # <><><>
            print("causalnex_dynotears_vanilla._learn_dynamic_structure: FINAL h_value == ", h_value, flush=True) # <><><>
            break
