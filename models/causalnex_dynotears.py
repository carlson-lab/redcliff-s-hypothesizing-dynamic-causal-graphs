# FROM THE AUTHOR(S): THE FOLLOWING CODE IS ADAPTED FROM https://github.com/mckinsey/causalnex/blob/develop/causalnex/structure/dynotears.py such that it performs stochastic gradient descent over mini-batches of a large dataset; aspects of the code which differ from the original implementation (which itself falls under the Apache 2.0 license) are marked with a '<><><>' comment
...
def from_pandas_dynamic(  # pylint: disable=too-many-arguments
    ...
    tabu_child_nodes: List[int] = None, grad_step=1.0, wa_est=None, rho=None, alpha=None, h_value=None, h_new=None, wa_new=None # <><><>
):# -> StructureModel:# <><><>
    """
    ...
    g, w_est, a_est, wa_est, rho, alpha, h_value, h_new, wa_new, n, d_vars, p_orders = from_numpy_dynamic( # <><><>
        X,
        Xlags,
        lambda_w,
        lambda_a,
        ...
        tabu_edges,
        tabu_parent_nodes,
        tabu_child_nodes, grad_step=grad_step, wa_est=wa_est, rho=rho, alpha=alpha, h_value=h_value, h_new=h_new, wa_new=wa_new # <><><>
    )
    ...
    return sm, w_est, a_est, wa_est, rho, alpha, h_value, h_new, wa_new, n, d_vars, p_orders # <><><>
...
def from_numpy_dynamic(  # pylint: disable=too-many-arguments
    ...
    bnds = bnds_w + bnds_a
    w_est, a_est, wa_est, rho, alpha, h_value, h_new, wa_new, n, d_vars, p_orders = _learn_dynamic_structure( # <><><>
        X, Xlags, bnds, lambda_w, lambda_a, max_iter, h_tol, grad_step=grad_step, wa_est=wa_est, rho=rho, alpha=alpha, h_value=h_value, h_new=h_new, wa_new=wa_new # <><><>
    )
...
def _learn_dynamic_structure(
    ...
    lambda_a: float = 0.1,
    max_iter: int = 100,
    h_tol: float = 1e-8, grad_step=1.0, wa_est=None, rho=None, alpha=None, h_value=None, h_new=None, wa_new=None # <><><>
):# -> Tuple[np.ndarray, np.ndarray]: # <><><>
    """
    ...
    def _grad(wa_vec: np.ndarray) -> np.ndarray:
        ...
        return grad_step*np.append(grad_vec_w, grad_vec_a, axis=0) # <><><>
    ...
    w_mat, a_mat = _reshape_wa(wa_est, d_vars, p_orders) # <><><>
    return w_mat, a_mat, wa_est, rho, alpha, h_value, h_new, wa_new, n, d_vars, p_orders # <><><>
...
def dynotears_objective(X, Xlags, wa_vec, rho, alpha, d_vars, p_orders, lambda_a, lambda_w, n): # <><><>
    """
    Objective function that the dynotears tries to minimise
    ...
    _h_value = dynotears_h_constraint(wa_vec, d_vars, p_orders) # <><><>
    l1_penalty = lambda_w * (wa_vec[: 2 * d_vars**2].sum()) + lambda_a * (
        wa_vec[2 * d_vars**2 :].sum()
    )
    return loss + 0.5 * rho * _h_value * _h_value + alpha * _h_value + l1_penalty